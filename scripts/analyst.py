...
- use_casesは「〜したい時」という形式で3つ
- stepsは実行手順を4ステップで簡潔に
- titleには必ずニュースで紹介されているAIモデル名/AIサービス名/AIツール名を含めること（例: 「GPT-5.2でコード生成を効率化」「NotebookLMで技術文書を要約」）

以下のJSON形式で出力してください。文字列内に改行を入れないでください：

{{"title": "戦術タイトル（[AI名]で○○する形式、30文字以内）", "problem_context": "解決する課題（40文字以内）", "recommended_ai": {{"model": "推奨AIモデル名", "reason": "推奨理由（30文字以内）", "badge_color": "orange"}}, "use_cases": ["〜したい時", "〜したい時", "〜したい時"], "steps": ["手順1", "手順2", "手順3", "手順4"], "prompt": "実際に使えるプロンプト（改行なし、150文字以内）", "tags": ["タグ1", "タグ2"]}}
